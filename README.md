...
1. Open **Settings** to choose the source/destination folders or **Edit categories…**. A review category is required.
2. Go to **Review files → Scan files**. The app waits for stable files, then suggests categories. Nothing moves during scanning. Use **Details & full paths** for exact locations and supporting text; expand the skipped-files section to see exclusions.
3. Adjust the **Move to** category and check files to select them. The **Needs review** filter shows ambiguous/unsupported items. Click **Sort selected…** to review the final destinations. **Approve & sort** is the single confirmation that authorizes the selected moves; Cancel leaves files untouched.
4. Open **History → Undo move** to restore a file. Expand **Locations** for original, sorted and restored paths. Undo pauses automation and refuses to move a sorted file that has changed.
5. For future arrivals, open **Automatic sorting** after scanning. Choose file types only or also include AI document suggestions, check **I have reviewed the current preview**, then **Turn on automatic sorting…** and confirm. Existing files remain excluded.

Automatic sorting initially moves only obvious file-type matches. Document suggestions require Apple Intelligence to be enabled in Settings and available on the Mac. Needs Review files always wait for manual approval. **Pause sorting** is available in the sidebar while automation runs. Monitoring stops when the app quits; each launch starts off and requires a fresh preview review. No login item or background service is installed.
